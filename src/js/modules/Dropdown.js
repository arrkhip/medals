...
export default class Dropdown extends ClassToggler {
  constructor(options) {
    ...
    this.$content = this.$el.querySelector('.j_dropdown__content') || null;
    this.$list = this.$el.querySelector('.j_dropdown__list') || null;
    this.$items = this.$list.querySelectorAll('input') || null;
    this.$search = this.$el.querySelector('[data-dropdown-search]') || null;

    this.placeholder = this.$title ? this.$title.getAttribute('data-placeholder') : '';
    ...
    this._isSoloSelect = this._isSelect && this.$items[0].type === 'radio';
  ...
  setSelectedTitle() {
    ...
    if (this._isSoloSelect) {
      this.text = this.titleTemplate.replace('$', `${this.$select.selectedOptions[0].textContent}`);
    } else {
      this.text = this.titleTemplate.replace('$', `<b>${this.$select.selectedOptions.length}</b>`);
    }
